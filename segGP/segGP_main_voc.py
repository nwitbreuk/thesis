...
elif DATASET == 'log_furex':
    dataSetName = 'log_furex'
    image_dir = "/dataB1/aoi/benchmarks/ensemble_datasets/log_furex/train/images"
    mask_dir = "/dataB1/aoi/benchmarks/ensemble_datasets/log_furex/train/masks"
elif DATASET == 'hikvision':
    dataSetName = 'hikvision'
    image_dir = "/dataB1/aoi/benchmarks/ensemble_datasets/hikvision/train/images"
    mask_dir = "/dataB1/aoi/benchmarks/ensemble_datasets/hikvision/train/masks"
else:
    raise ValueError(f"Unknown dataset option: {DATASET}")
...
def _scan_unique_ids(ds, limit=64):
    ...
    for i in range(min(limit, len(ds))):
        _, m = ds[i]
        mt = torch.as_tensor(np.array(m)) if not isinstance(m, torch.Tensor) else m
        if mt.dim() == 3 and mt.shape[0] == 1:
            mt = mt.squeeze(0)
        for v in torch.unique(mt).cpu().tolist():
            ids.add(int(v))
    return sorted(ids)

orig_ids = _scan_unique_ids(dataset)
background_present = 0 in orig_ids
EXCLUDE_BACKGROUND = background_present
BACKGROUND_CLASS = 0 if background_present else DEFAULT_IGNORE_INDEX
print(f"[Dataset] Detected mask ids: {orig_ids} | exclude_background={EXCLUDE_BACKGROUND}")
...
# When running baselines-only, pass background flags through
if BASELINE_ONLY:
...
pset.addPrimitive(
    tp("NormalizeClasses", lambda x: felgp_fs.normalize_to_selected_classes(x, SELECTED_CLASSES)), # type: ignore
    [seg_types.FeatureMap], seg_types.FeatureMap, name="NormalizeClasses"
)
